use windows::Win32::System::{
    JobObjects::{
        ...
        JOB_OBJECT_LIMIT_PROCESS_MEMORY, JOB_OBJECT_LIMIT_SCHEDULING_CLASS,
...
impl ExtendedLimitInfo {
    ...
    /// Limits the committed memory of each process associated with the job.
    pub fn limit_process_memory(&mut self, maximum: usize) -> &mut Self {
        self.0.ProcessMemoryLimit = maximum;
        self.0.BasicLimitInformation.LimitFlags |= JOB_OBJECT_LIMIT_PROCESS_MEMORY;

        self
    }

    /// Limits the aggregate committed memory of every process associated with the job.
    pub fn limit_job_memory(&mut self, maximum: usize) -> &mut Self {
        self.0.JobMemoryLimit = maximum;

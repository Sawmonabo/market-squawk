...
use crate::model::{DateError, FeedKind, FeedVersion, Sha256Digest, TradeDate, TransportVersion};
...
/// Trusted transport metadata paired with one exact catalog body.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CatalogTransportMetadata {
    ...
    /// Local retrieval-completion time in Unix nanoseconds.
    pub retrieved_at_unix_nanos: i64,
    /// Local calendar date used for T+1 and rolling-window admission.
    pub observed_on: TradeDate,
}
...
pub struct CatalogReceipt {
    ...
    pub etag: Option<String>,
    /// Local retrieval-completion time.
    pub retrieved_at_unix_nanos: i64,
    /// Local date on which this mutable catalog was observed.
    pub observed_on: TradeDate,
}
...
pub struct SelectedFileReceipt {
    ...
    /// Catalog observation date controlling T+1/rolling admission.
    pub(crate) catalog_observed_on: TradeDate,
}
...
impl SelectedFileReceipt {
    ...
    pub fn identity(&self) -> Sha256Digest {
        digest_fields(&[
            b"market-squawk/iex-hist-selected-file/v1",
...
impl Catalog {
    ...
    pub fn parse(body: &[u8], metadata: CatalogTransportMetadata) -> Result<Self, CatalogError> {
        ...
        let receipt = CatalogReceipt {
            ...
            etag: metadata.etag,
            retrieved_at_unix_nanos: metadata.retrieved_at_unix_nanos,
            observed_on: metadata.observed_on,
        };
    ...
    pub fn select(&self, request: &ExactFileRequest) -> Result<SelectedFileReceipt, CatalogError> {
        ...
        Ok(SelectedFileReceipt {
            ...
            catalog_observed_on: self.receipt.observed_on,
        })
    }
}
...
fn validate_transport(
    ...
    if metadata.status != 200
        || metadata.retrieved_at_unix_nanos < 0
...
fn validate_descriptor(
    ...
    let descriptor_sha256 = digest_fields(&[
        b"market-squawk/iex-hist-catalog-descriptor/v1",

...
| [`argon2`](https://docs.rs/crate/argon2/0.5.3) | `=0.5.3`, `default-features = false`, `features = ["alloc"]` | MIT OR Apache-2.0; Rust 1.65 | Stable Argon2id KDF for the encrypted local fallback. Parameters, version, random salt, and memory/time/parallelism limits are persisted; derived keys are zeroized. Do not adopt the 0.6 release candidate. |

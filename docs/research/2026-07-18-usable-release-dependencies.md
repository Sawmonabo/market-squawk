...
| [`atomicwrites`](https://docs.rs/crate/atomicwrites/0.4.4) | `=0.4.4`, admitted only under `cfg(windows)` for `market-squawk-data` | MIT; crate MSRV undeclared; last release/code activity in 2024 | Supplies one audited safe boundary for no-clobber, write-through catalog-backup publication while the workspace retains `unsafe_code = "forbid"`. Its Windows [`move_atomic`](https://docs.rs/crate/atomicwrites/0.4.4/source/src/lib.rs) accepts `Path` values and calls `MoveFileExW` with `MOVEFILE_WRITE_THROUGH` but not replacement or cross-volume-copy flags; Market Squawk still owns close-before-publish, root and endpoint revalidation, writer exclusion, ambiguous-result reconciliation, and receipt-bound size/digest/SQLite verification. The crate lacks active maintenance and upstream Windows CI, and [does not establish atomic visibility](https://github.com/untitaker/rust-atomicwrites/issues/27); those accepted residual risks prohibit authoritative in-place replacement. The target-only graph adds `windows-sys` 0.52; exact Rust 1.97.1 Windows build/test evidence and source re-audit on every version change are admission gates. |

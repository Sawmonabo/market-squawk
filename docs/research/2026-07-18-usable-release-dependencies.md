...
5. Every provider uses one authorization identity and one process-authoritative budget. Caching,
   retries, bulk endpoints, and failover are resilience mechanisms.
...
| [`tract-onnx`](https://docs.rs/crate/tract-onnx/0.23.4) | `=0.23.4`, `default-features = false` | Crate: MIT OR Apache-2.0; transitive `dyn-eq` 0.1.3: MPL-2.0; Rust 1.91 | Required self-contained ONNX-compatible backend. It has no default feature or native runtime download. Bundle admission allowlists supported operators, dimensions, tensor bytes, model bytes, threads, warm-up, and deadlines; any parse or inference error produces no automated action. The MPL dependency is admitted only through an exact `cargo-deny` exception and the [required tract notice](../licenses/tract-onnx-notice.md), which records source and executable-distribution obligations. |
...
1.91. Its mandatory graph includes [`dyn-eq` 0.1.3](https://github.com/Rayzeq/dyn-eq/commit/674274e2aded185c0b4f54cc9745c844bce5fca9),
whose manifest and license identify MPL-2.0; Mozilla's
[MPL FAQ](https://www.mozilla.org/en-US/MPL/2.0/FAQ/) confirms that an executable larger work may
use other terms while recipients retain access to the MPL-covered source. `ort` 2.0.0-rc.12 remains
the latest published `ort` release. The

...
| [`atomicwrites`](https://docs.rs/crate/atomicwrites/0.4.4) | `=0.4.4`, admitted only under `cfg(windows)` for `market-squawk-data` | MIT; crate MSRV undeclared | Supplies one audited safe boundary for no-clobber, write-through catalog-backup publication while the workspace retains `unsafe_code = "forbid"`. Its Windows [`move_atomic`](https://docs.rs/crate/atomicwrites/0.4.4/source/src/lib.rs) accepts `Path` values and calls `MoveFileExW` with `MOVEFILE_WRITE_THROUGH` but not `MOVEFILE_REPLACE_EXISTING`; Market Squawk still closes write handles, retains destination writer exclusion, validates endpoints and identity, and rejects publication failure. The target-only graph adds `windows-sys` 0.52; exact Rust 1.97.1 Windows build/test evidence and source re-audit on every version change are admission gates. |

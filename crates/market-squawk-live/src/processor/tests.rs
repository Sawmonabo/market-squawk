...
fn processor(
    ...
    processor_with_lifetime(clock, nonce_capacity, Duration::from_secs(120))
}
...
fn rejected_new_stream_does_not_consume_source_generation_capacity() -> TestResult {
    ...
    let evaluated_at = first
        .timestamp(EVALUATED_AT)?
        .max(second.timestamp(EVALUATED_AT)?);
    let first_admission = registry.bind_current(&first_lease, evaluated_at)?;
    let second_admission = registry.bind_current(&second_lease, evaluated_at)?;
    let (mut processor, _shard, _runtime) = processor_with_capacities(
        fixed_clock(evaluated_at)?,
        1,
        2,
        4,
...
fn failed_generation_replacement_preserves_the_former_processor_state() -> TestResult {
    ...
    let second_evaluated_at = source.timestamp(EVALUATED_AT)?;
    let second_admission = registry.bind_current(&second_lease, second_evaluated_at)?;
    ...
    let former_revision = processor
        ...
        .ok_or("missing former stream")?
        .revision();
    processor.clock = fixed_clock(second_evaluated_at)?;
    let mut replacement = processor.accept_batch(second_batch, &second_admission)?;
    let base_mono = Instant::now();
    let current = ClockReading::new(second_evaluated_at, base_mono);
    let expired = ClockReading::new(
        source.timestamp(VALID_UNTIL + 1)?,
        base_mono
...
fn ready_trade(nonce_capacity: usize) -> TestResult<ReadyTrade> {
    ready_trade_with_clock(nonce_capacity, |source| {
        fixed_clock(source.timestamp(EVALUATED_AT)?)
    })
}

fn ready_trade_with_clock(
    nonce_capacity: usize,
    build_clock: impl FnOnce(&SourceHarness) -> TestResult<ScriptedTrustedClock>,
) -> TestResult<ReadyTrade> {
    let mut source = SourceHarness::try_new("source-a", 1)?;
    let (lease, batch) = source.batch("trade-1", 1, trade()?, non_book_snapshot()?)?;
    let mut registry = GenerationAuthorityRegistry::try_new(2)?;
    let admission = registry.bind_current(&lease, source.timestamp(EVALUATED_AT)?)?;
    let clock = build_clock(&source)?;
...
fn generation_registry_rebinds_refresh_and_replaces_rollover_atomically() -> TestResult {
    ...
    let successor = registry.bind_current(&successor_lease, source.timestamp(HEALTH_AT + 3)?)?;
    assert!(
        !refreshed
            .generation()
    ...
    successor.validate_at(source.timestamp(HEALTH_AT + 3)?)?;
    Ok(())
}
...
fn generation_registry_rejects_transplant_capacity_and_exit_resurrection() -> TestResult {
    ...
    let mut registry = GenerationAuthorityRegistry::try_new(1)?;
    let first_at = first_source.timestamp(HEALTH_AT)?;
    let admission = registry.bind_current(&first_lease, first_at)?;
    assert!(matches!(
        registry.bind_current(
            &transplanted_lease,
            transplanted_source.timestamp(HEALTH_AT)?
        ),
        Err(LiveApplyError::GenerationAdmissionTransplant)
    ));
    assert!(matches!(
        registry.bind_current(&second_lease, second_source.timestamp(HEALTH_AT)?),
    ...
    exit.invalidate();
    assert!(matches!(
        admission.validate_at(first_at),
        Err(LiveApplyError::Authority(AuthorityError::Revoked))
    ));
    assert!(matches!(
        registry.bind_current(&first_lease, first_at),
        Err(LiveApplyError::Authority(AuthorityError::Revoked))
    ));
    Ok(())
}
...
fn committed_seed_is_sorted_truncated_and_exactly_base_charged() -> TestResult {
    ...
    let admission_a = registry.bind_current(&lease_a, evaluated_at)?;
    let (mut processor, _shard, _runtime) = processor(fixed_clock(evaluated_at)?, 4)?;
...
fn late_delta_expiry_rolls_back_book_sequence_revision_and_status() -> TestResult {
    let mut source = SourceHarness::try_new("source-a", 1)?;
    let base_mono = Instant::now();
    let current = ClockReading::new(source.timestamp(EVALUATED_AT)?, base_mono);
    let expired = ClockReading::new(
        source.timestamp(VALID_UNTIL + 1)?,
        base_mono
            .checked_add(Duration::from_nanos(u64::try_from(
...
fn consumed_authority_rechecks_runtime_and_processor_exit() -> TestResult {
    ...
    assert_eq!(
        consumed.validate_at_for_test(ClockReading::new(
            ready.source.timestamp(EVALUATED_AT)?,
            Instant::now(),
        )),
        Err(AuthorityError::Revoked)
    );

    let mut exit_ready = ready_trade(2)?;
...
fn inclusive_deadline_passes_and_plus_one_nanosecond_fails_queued_issue() -> TestResult {
    let mut ready = ready_trade_with_clock(2, |source| {
        let base_mono = Instant::now();
        ...
        let expired_mono = deadline_mono
            ...
            .ok_or("expired instant overflow")?;
        let current = ClockReading::new(source.timestamp(EVALUATED_AT)?, base_mono);
        let deadline = ClockReading::new(source.timestamp(VALID_UNTIL)?, deadline_mono);
        Ok(ScriptedTrustedClock::try_new(vec![
            current,
            current,
            current,
            deadline,
            deadline,
            deadline,
            ClockReading::new(source.timestamp(VALID_UNTIL + 1)?, expired_mono),
        ])?)
    })?;
    ready.processor.validate_applied_current(&ready.applied)?;

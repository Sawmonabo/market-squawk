...
use market_squawk_domain::{
    ...
    CaptureInitializer, ConnectionGeneration, MetadataRevision, RawCaptureFrameView, SourceId,
...
-> Result<(), Box<dyn std::error::Error>> {
    let (mut initializer, mut admission, degradation) = TestBundle::new().into_parts();
    let frame = TestFrame {

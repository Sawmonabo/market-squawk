//! Closed adapter over the feature-gated production benchmark seam.
...
use super::schema::ForcedLockResult;
...
pub(crate) fn run_forced_lock() -> Result<Option<ForcedLockResult>, Box<dyn std::error::Error>> {
    if REQUIRES_BASELINE {
        return Err("candidate backend must implement its forced-lock evidence fixture".into());
    }
    Ok(None)
}
